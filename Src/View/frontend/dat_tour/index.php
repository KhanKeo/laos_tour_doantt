    ...
    <div class="row">
        ...
        <div class="col-sm-10">
            ...
            <ul class="nav nav-tabs">
                ...
                <li role="presentation"><a href="/dattour/finishedUser/<?= $tour['id'] ?>">Đã hoàn thành</a></li>
                ...
                <div class="row">
                    ...
                    <div class="col-sm-6">
                        <form id="search" style="display: flex; justify-content: end;">
                            ...
                            if ($search != '') {
                            ?>
                                <button type="button" onclick="location.href='/<?= $controller ?>/indexUser/<?= $tour['id'] ?>'" class="btn btn-white"><i class="fa fa-repeat"></i></button>
                            <?php
                            }
                            ?>
            ...
            <nav aria-label="Page navigation example">
                <ul class="pagination d-flex justify-content-center">
                    ...
                    ?>
                        <li class="page-item <?= $results['page'] == $index ? 'active' : '' ?>"><a class="page-link" href="/<?= $controller ?>/indexUser/<?= $tour['id'] ?>?page=<?= $index ?>"><?= $index ?></a></li>
                    <?php
                    }
                    ?>
                </ul>
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                ...
                <div class="form-group">
                    <div class="col-sm-8">
                        <input type="text" class="form-control" name="loi_nhac" id="loi_nhac_huy" placeholder="Lời nhắc" required>
                    </div>
                </div>
            </div>
    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            ...
            <div class="modal-body form-horizontal">
                ...
                <div class="form-group">
                    <div class="col-sm-8">
                        <input type="text" class="form-control" name="loi_nhac" id="loi_nhac_xac_nhan" placeholder="Lời nhắc" required>
                    </div>
                </div>
            </div>
            <div class="modal-footer">
...
<script>
    ...
    $('#search').submit(() => {
        ...
        redirect('/' + controller + '/indexUser/<?= $tour['id'] ?>?page=<?= $results['page'] ?>' + '&search=' + search);
    });
    ...
    const confirmCancel = () => {
        $.ajax({
            ...
            data: {
                loi_nhac: $('#loi_nhac_huy').val(),
            },
            dataType: 'json',
            success: (data) => {
                if (data.code === 'cancel_success') {
                    redirect('/' + controller + '/indexUser/<?= $tour['id'] ?>', 'Hủy thành công');
                }
            },
            error: (xhr) => {
                $('#cancel-button').prop('disabled', false);
                $('#cancel-button').html('Hủy');
                redirect('/' + controller + '/indexUser/<?= $tour['id'] ?>', 'Hủy không thành công', 'error');
            }
        });
    ...
    const confirmAccept = () => {
        $.ajax({
            ...
            dataType: 'json',
            data: {
                loi_nhac: $('#loi_nhac_xac_nhan').val(),
            },
            success: (data) => {
                ...
                if (data.code === 'confirm_success') {
                    redirect('/' + controller + '/indexUser/<?= $tour['id'] ?>', 'Xác nhận thành công');
                }
            },
            error: (xhr) => {
                $('#confirm-button').prop('disabled', false);
