    ...
    <div class="row">
        ...
        <div class="col-sm-10">
            ...
            <ul class="nav nav-tabs">
                ...
                <li role="presentation" class="active"><a>Đang thực hiện</a></li>
                ...
                <div class="row">
                    ...
                    <div class="col-sm-6">
                        <form id="search" style="display: flex; justify-content: end;">
                            ...
                            if ($search != '') {
                            ?>
                                <button type="button" onclick="location.href='/<?= $controller ?>/progressUser/<?= $tour['id'] ?>'" class="btn btn-white"><i class="fa fa-repeat"></i></button>
                            <?php
                            }
                            ?>
                            ...
                            <td>
                                <div>
                                    <a onclick="accept(<?= $result['id'] ?>)">
                                        <button type="button" class="btn btn-primary btn-block" style="display:inline;width:auto;margin-left:10px;">Xác nhận đã hoàn thành</button>
                                    </a>
            ...
            <nav aria-label="Page navigation example">
                <ul class="pagination d-flex justify-content-center">
                    ...
                    ?>
                        <li class="page-item <?= $results['page'] == $index ? 'active' : '' ?>"><a class="page-link" href="/<?= $controller ?>/progressUser/<?= $tour['id'] ?>?page=<?= $index ?>"><?= $index ?></a></li>
                    <?php
                    }
                    ?>
                </ul>
...
<script>
    ...
    $('#search').submit(() => {
        ...
        redirect('/' + controller + '/progressUser/<?= $tour['id'] ?>?page=<?= $results['page'] ?>' + '&search=' + search);
    });
        ...
        $.ajax({
            ...
            success: (data) => {
                if (data.code === 'destroy_success') {
                    redirect('/' + controller + '/progressUser/<?= $tour['id'] ?>', 'Xóa thành công');
                }
            },
            error: (xhr) => {
                $('#cancel-button').prop('disabled', false);
                $('#cancel-button').html('Hủy');
                redirect('/' + controller + '/progressUser/<?= $tour['id'] ?>', 'Xóa không thành công', 'error');
            }
        })
        ...
        $.ajax({
            ...
            success: (data) => {
                ...
                if (data.code === 'confirm_success') {
                    redirect('/' + controller + '/progressUser/<?= $tour['id'] ?>', 'Xác nhận thành công');
                }
            },
            error: (xhr) => {
                $('#confirm-button').prop('disabled', false);
