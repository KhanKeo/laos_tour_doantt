...
<nav aria-label="Page navigation example">
    <ul class="pagination d-flex justify-content-center">
        ...
        ?>
            <li class="page-item <?= $results['page'] == $index ? 'active' : '' ?>"><a class="page-link" href="/<?= $controller ?>/progress?page=<?= $index ?>"><?= $index ?></a></li>
        <?php
        }
        ?>
    </ul>
    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            ...
            <div class="modal-footer">
                <button type="button" class="btn btn-primary" onclick="confirmCancel()">Xóa</button>
...
<div class="modal fade" id="accept" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            ...
            <div class="modal-footer">
                <button id="confirm-button" type="button" class="btn btn-primary" onclick="confirmAccept()">Xác nhận hoàn thành</button>
                <button type="button" class="btn btn-light" data-bs-dismiss="modal">Hủy</button>
            </div>
        </div>
    </div>
...
<script>
    ...
    $('#search').submit(() => {
        ...
        redirect('/' + controller + '/progress/?page=<?= $results['page'] ?>' + '&search=' + search);
    });
        ...
        $.ajax({
            ...
            success: (data) => {
                if (data.code === 'destroy_success') {
                    redirect('/' + controller + '/progress', 'Xóa thành công');
                }
            },
            error: (xhr) => {
                redirect('/' + controller + '/progress', 'Xóa không thành công', 'error');
            }
        })
    }
    ...
    const confirmAccept = () => {
        $.ajax({
            ...
            success: (data) => {
                ...
                $('#confirm-button').html('Xác nhận hoàn thành');

                if (data.code === 'confirm_success') {
                    redirect('/' + controller + '/progress', 'Xác nhận thành công');
                }
            },
            error: (xhr) => {
                $('#confirm-button').prop('disabled', false);
